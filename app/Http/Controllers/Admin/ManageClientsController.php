...
class ManageClientsController extends Controller
{
    ...
    {      
        ...
        $data['rows'] = User::when($q, function($query) use ($q){                           
                        ...
                        })->where('user_type', 1)->orderBy($filter_clumn,$sorting)->paginate($limit);
    ...
    function postAddSave(Request $request)
    {
        ...
        {
            return redirect($return_url)->withSuccess('Client added successfully!');
        }else{
            return redirect()->back()->withSuccess('Client added successfully!');
        }
    }
    ...
    function postUpdateSave($id, Request $request)
    {
      if (!AdminHelper::isUpdate()) {
        ...
        $return_url = (!empty($request->input('return_url'))?$request->input('return_url'):route('getUserIndex'));
        return redirect($return_url)->withSuccess('Client updated successfully!');
    }
    ...
    function getDetail($id)
    {
        ...
        return view('admin.clients.detail', $data);
    }
    ...
    function getDelete($id)
    {
        ...
        if(!empty($id))
        {
            ...
            return redirect()->back()->withSuccess('Client deleted successfully!');
        }
    }
