...
class ManageBanners extends Controller
{
    ...
    {      
        ...
        if($filter_clumn!='')
        {
             $data['rows'] = ManageBanner::leftJoin('cms_pages', 'cms_pages.id', '=', 'manage_banners.page')->where('manage_banners.is_delete','0')->when($q, function($query) use ($q){                          
                           $query->whereRaw("( `manage_banners`.`name` like '%".$q."%' or `cms_pages`.`page_title` like '%".$q."%')");
    ...
    function postAddSave(Request $request)
    {
        ...
        $isdata = ManageBanner::where('name' ,$request->input('name'))->where('page' ,$request->input('page'))->where('is_delete','0')->get();
    	...
    	{
    		return redirect($return_url)->withSuccess('Image added successfully!');
    	}else{
    ...
    function postUpdateSave($id, Request $request)
    {
       ...
       /* if($request->input('link'))
        ...
        //$data->banner_type = $request->input('banner_type');
    	...
    	$return_url = (!empty($request->input('return_url'))?$request->input('return_url'):AdminHelper::adminPath('admin').'/banner');
    	return redirect($return_url)->withSuccess('Banner updated successfully!');
    }
