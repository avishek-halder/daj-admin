...
use App\Country;
class CountryPin implements Rule
{
/**
...
'ZM' => '/^(?:\d{5})$/i',
'ZW' => null];
if($country_id>0 && $pin_code!='')
{
	$countr_dtls = Country::where('id',$country_id)->first();
	$country_shotcode = $countr_dtls->sortname;
	if(isset($allpreg[$country_shotcode])){
        $pattern = $allpreg[$country_shotcode];
        return preg_match($pattern, $pin_code);
    }else{
       $pattern = null;
       return 1;
   	}
...
{
return 'This is not a valid pin code in respect of the country.';
}
}