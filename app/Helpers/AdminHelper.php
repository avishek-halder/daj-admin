...
class AdminHelper
{
    ...
    {
        $roles = DB::table('admin_privileges_roles')
        ->where('id_admin_privileges', self::myPrivilegeId())
        ->join('admin_menus', 'admin_menus.id', '=', 'id_admin_menus')->select('admin_menus.name', 'admin_menus.path', 'admin_menus.type', 'is_visible', 'is_create', 'is_read', 'is_edit', 'is_delete')
        ->get();
        return $roles;
    }
    ...
    public static function sendEmail($config = [])
    {
        ...
        \Mail::send("admin.emails.email_template", ['content' => $html], function ($message) use ($to, $subject, $template, $attachments) {
            $message->priority(1);
            $message->to($to);
